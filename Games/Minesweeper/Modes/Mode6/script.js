const boardSize = 24; // Updated board size to 24
const numberOfMines = 100; // Total number of mines
let board = [];
let revealedCount = 0;
...
function revealAdjacentCells(x, y) {
    ...
    for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
            const newX = x + i;
            const newY = y + j;
            // Check if the cell is within bounds and not revealed
            if (newX >= 0 && newX < boardSize && newY >= 0 && newY < boardSize) {
                const neighbor = board[newX][newY];
                if (!neighbor.revealed && !neighbor.mine) {
                    neighbor.revealed = true; // Reveal the neighboring cell
                    revealedCount++; // Increment revealed count
                    if (neighbor.value === 0) {
                        revealAdjacentCells(newX, newY); // Recursively reveal cells if value is zero
                    }
                }
            }
        }
    }
    renderBoard(); // Re-render the board after revealing adjacent cells
    checkWinCondition(); // Check if the player has won
}
...
function checkWinCondition() {
    ...
    if (revealedCount === boardSize * boardSize - numberOfMines) {
        showToast('Congratulations! You win!'); // Show toast if player wins
        setTimeout(() => {
            resetGame(); // Reset the game after a short delay
...
function resetGame() {
    ...
    document.getElementById('minesCount').innerText = minesLeft; // Update the mines count
}

document.getElementById('resetBtn').addEventListener('click', resetGame); // Event listener for reset button
